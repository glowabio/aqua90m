...
curl -X POST https://$PYSERVER/processes/get-basin-subcatchments/execution \
...
class BasinSubcatchmentsGetter(GeoFreshBaseProcessor):
    ...
    def _execute(self, data, requested_outputs, conn):
        ...
        if geometry_only:
            ...
            geojson_collection = get_polygons.get_subcatchment_polygons_feature_coll_by_basin(
                conn, basin_id, reg_id)
...
if __name__ == '__main__':
    ...
    process_id = 'get-basin-subcatchments'
    ...
    print('TEST CASE 4: Input: point, output: GeometryCollection...', end="", flush=True)  # no newline
    payload = {
        "inputs":
            {
            "point": {
    ...
    print('TEST CASE 6: Input: point, output: FeatureCollection...', end="", flush=True)  # no newline
    payload = {
        "inputs":
            {
            "point": {
